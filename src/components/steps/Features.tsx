...
import { featuresSchema } from"@/lib/schema";
import { z } from"zod";
...
export function Features() {
  ...
  const { handleSubmit, setValue, watch } = useForm<FeaturesData>({
  ...
  const onSubmit = (data: FeaturesData) => {
    updateData(data);
    nextStep();
  };
  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        ...
        <p className="text-gray-600">Select the features you want to include on your website.</p>
      </div>
          ...
          {featuresList.map((feature) => {
            ...
            return (
              <button
                ...
                className={`p-3 rounded-xl border-2 text-sm font-medium transition-all duration-200 text-left flex items-center space-x-2 ${
                  isSelected 
